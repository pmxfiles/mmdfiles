...
float4 Normal_PS(VS_OUTPUT IN,uniform bool useTexture) : COLOR0
{
    ...
    float2 tex = IN.Tex* MapParam;
	float4 Color;
	float3 normal;
	float height = 0;
	
	height = tex2D( HeightMapSamp, IN.Tex * MapParam).r;
...
float4 BufferShadow_Mec_PS(BufferShadow_OUTPUT IN, uniform bool useTexture, uniform bool useSphereMap, uniform bool useToon) : COLOR
{
	...
	IN.Normal = (tex2D(NormalSampler,ScrTex).rgb*2-1);
	//9点サンプリング
	for(int i=0;i<8;i++)
	{
		IN.Normal += tex2D(NormalSampler,ScrTex+test[i]*SampStep).rgb*2-1;
	}
	IN.Normal /= 9;
