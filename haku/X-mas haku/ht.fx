////////////////////////////////////////////////////////////////////////////////////////////////
//　ピクセルパーライティングシェーダー
//		改造元　full.fx ver1.2　(作成: 舞力介入P)
////////////////////////////////////////////////////////////////////////////////////////////////
// 修正はobject_ss部分のみで実験

const float3 LightPosition = float3( 100, 100, -200); //** 点光源位置
const float  SpecularShininess = 64; //** スペキュラパラメータ SpecularPowerを使うべきか？
const float3 SpecularWhiteColor = float3(0.5, 0.5, 0.5); //** 白いスペキュラカラー。SpecularColorが本来色
const float3 myLightDiffuse = float3(0.8, 0.8, 0.8);
const float3 myLightAmbient = float3(0.7, 0.7, 0.7);
...
BufferShadow_OUTPUT BufferShadow_VS(float4 Pos : POSITION, float3 Normal : NORMAL, float2 Tex : TEXCOORD0, uniform bool useTexture, uniform bool useSphereMap, uniform bool useToon)
{
    ...
    // カメラ視点のワールドビュー射影変換
    Out.Pos = mul( Pos, WorldViewProjMatrix );
    Out.WPos= mul( Pos, WorldMatrix ); //** ワールド位置
...
{
	float3 pos = LightPosition-IN.WPos.xyz; //** ライトへのベクトル
	float lc = dot(normalize(pos) , normalize(IN.Normal));// * IN.Color; //** カラー

    // スペキュラ色計算
    float3 HalfVector = normalize( normalize(IN.Eye) + normalize(pos)  ); //** ライトへのハーフベクトル
    float3 Specular = pow( max(0,dot( HalfVector, normalize(IN.Normal) )), SpecularShininess )*SpecularWhiteColor; //** スペキュラ
    
    float4 Color = IN.Color;

   // Color.xyz = lc*0.5  + AmbientColor * 0.5;
 
    float3 myAmbientColor = MaterialAmbient * myLightAmbient;
    
    Color.xyz = MaterialDiffuse * myLightDiffuse * lc + myAmbientColor ;

    float4 ShadowColor = float4(MaterialDiffuse.xyz, Color.a);  // 影の色
    if ( useTexture ) {
        // テクスチャ適用
    ...
    if( any( saturate(TransTexCoord) != TransTexCoord ) ) {
		...
		//ShadowColor.rgb = Color.xyz * 0.8; //**影の色は現在色を暗くしたものを使用したら...
        
        float4 ans = lerp(ShadowColor, Color, comp);
        ans.rgb += Specular.rgb * comp;//** 影の部分にはスペキュラを加算しない
